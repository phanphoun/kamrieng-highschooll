        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                <div class="text-center p-6 bg-gray-50 rounded-xl">
                    ...
                    <p class="text-gray-600 text-sm">{{ __('messages.donate_tier_1') ?? 'Supplies for one student' }}</p>
                </div>
                <div class="text-center p-6 bg-primary-50 rounded-xl border-2 border-primary-200">
                    ...
                    <p class="text-gray-600 text-sm">{{ __('messages.donate_tier_2') ?? 'Funds a scholarship for a month' }}</p>
                </div>
                <div class="text-center p-6 bg-gray-50 rounded-xl">
                    <div class="text-3xl font-bold text-primary-600 mb-2">$100</div>
                    <p class="text-gray-600 text-sm">{{ __('messages.donate_tier_3') ?? 'Supports a lab or library' }}</p>
                </div>
            </div>

            <div class="text-center p-8 bg-gray-50 rounded-xl">
                <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ __('messages.bank_transfer') ?? 'Bank Transfer' }}</h3>
                <p class="text-gray-600 mb-2">{{ __('messages.bank_name') ?? 'Cambodia Bank PLC' }}</p>
                <p class="text-gray-600 mb-2">{{ __('messages.account_name') ?? 'Account Name: EduKhmer High School' }}</p>
                <p class="text-gray-600 mb-2">{{ __('messages.account_number') ?? 'Account Number: 1234-5678-9012' }}</p>
                <p class="text-gray-500 text-sm mt-4">{{ __('messages.donate_note') ?? 'Please include your name and contact information with your donation.' }}</p>
            </div>

            <div class="text-center mt-8">
                <a href="{{ route('contact') }}" class="inline-block px-8 py-3 bg-primary-600 text-white font-semibold rounded-lg hover:bg-primary-700 transition">
